...
public class BetRequestProjects {
    ...
    /**
     * @param game cxg360ffc qiqutxffssc
     * @param betMoneyUnit　下注金额单位　元角分厘
     * @param betWhere　　　　下注位　个十百千万
     * @param betMultiple　　　下注倍数　
     * @return
     */
    public static BetRequestProjects generate(String game , String betMoneyUnit , String betWhere,int betMultiple){
        String code = null;
        if("cxg360ffc".equals(game)){
        ...
        }else if("qiqutxffssc".equals(game)){
            if("个".equals(betWhere)){
                code = "||||1&2&3&4&5";
            }else if("十".equals(betWhere)){
                code = "|||1&2&3&4&5|";
            ...
            if("元".equals(betMoneyUnit)){
                total = betMultiple;
                defaultPrize =  Arrays.asList(new Double[]{ betMultiple * 9.95d});
                mode = 5;
            }else if("角".equals(betMoneyUnit)){
                total = betMultiple / 10d ;
                defaultPrize =  Arrays.asList(new Double[]{ betMultiple * 0.95d});
                mode = 6;
            }else if("分".equals(betMoneyUnit)){
                total = betMultiple / 100d ;
                defaultPrize =  Arrays.asList(new Double[]{ betMultiple * 0.095d});
                mode = 7;
            }else if("厘".equals(betMoneyUnit)){
                total = betMultiple / 1000d ;
                defaultPrize =  Arrays.asList(new Double[]{ betMultiple * 0.0095d});
                mode = 8;
            }

            DecimalFormat format = new DecimalFormat("0.0000");
            String totalStr = format.format(total *  5);
